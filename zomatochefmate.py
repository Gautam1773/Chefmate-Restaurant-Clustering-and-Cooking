        ...
        if not recommendations.empty and all(col in recommendations.columns for col in ['Restaurant_name', 'Cuisines', 'Aggregate_rating', 'City', 'Average_Cost_for_two', 'Currency']):
            for index, row in recommendations.iterrows():
                ...
                <div class="recommendation">
                    ...
                    <p><strong>Rating:</strong> {row['Aggregate_rating']} ⭐</p>
                    <p><strong>Location:</strong> {row['City']}</p>
                    <p><strong>Average Cost for Two:</strong> {row['Average_Cost_for_two']} {row['Currency']}</p>
                </div>
                """, unsafe_allow_html=True)
                # Add Thumbs-Up or Thumbs-Down buttons for feedback
                thumbs_up = st.button(f"👍 {row['Restaurant_name']} - Like", key=f"thumbs_up_{index}")
                thumbs_down = st.button(f"👎 {row['Restaurant_name']} - Dislike", key=f"thumbs_down_{index}")
                if thumbs_up:
                    st.write(f"You liked {row['Restaurant_name']}!")
                if thumbs_down:
                    st.write(f"You disliked {row['Restaurant_name']}.")
